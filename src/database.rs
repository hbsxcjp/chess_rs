...
pub fn insert_zorbist_evaluation(
    conn: &mut Connection,
    zorbist_eval: &evaluation::Zorbist,
) -> Result<()> {
...
pub fn get_zorbist_evaluation(conn: &mut Connection) -> evaluation::Zorbist {
    let zorbist_fields = zorbist_fields().join(", ");
    ...
    for row in row_iter {
        if let Ok((key, lock, from_index, to_index, count)) = row {
            zorbist.insert(
                key,
                lock,
                evaluation::Aspect::from_values(from_index, to_index, count),
            );
        }
    }
...
pub fn get_zorbist_evaluation_rowcols(conn: &mut Connection, cond: &str) -> evaluation::Zorbist {
